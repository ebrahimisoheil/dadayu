...
aggregated AS (
    ...
    FROM base
    GROUP BY score_date, market, sector, industry
),
...
SELECT
    *,
    ...
    ) AS industry_rank_overall,
    rank() OVER (
        PARTITION BY score_date, market
        ORDER BY avg_total_score DESC NULLS LAST, rankable_count DESC, industry ASC
    ) AS industry_rank_in_market,
    rank() OVER (
        PARTITION BY score_date, market, sector
        ORDER BY avg_total_score DESC NULLS LAST, rankable_count DESC, industry ASC
    ) AS industry_rank_in_sector
FROM scored
